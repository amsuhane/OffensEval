...
def load_data():
    ...
    data_training=pd.read_csv('training-v1/offenseval-training-v1.tsv',delimiter='\t',encoding='utf-8')
...
def make_model(X, max_sentence_len, one_hot_subtask_a):
    ...
    #checkpoint
    filepath="weights-improvement-a-biLSTM-fasttextpretrained-{epoch:02d}-{val_acc:.2f}.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
    callbacks_list = [checkpoint]

    #fit the model
    model_Bi_LSTM.fit(x=X, y=one_hot_subtask_a, validation_split=0.33,	batch_size=64, epochs=1, callbacks=callbacks_list, verbose=0)
    ...
    plt.title("BiLSTM (subtask_a) model (loss='categorical_crossentropy', optimizer='Adam', metrics=['accuracy']) \n fasttext pre-trained-vectors")
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.legend()
    plt.subplot(1, 2, 2)
    ...
    plt.ylabel("acc")
    plt.legend()
    plt.savefig('Bi_LSTM_a | fasttext_trained' + str(TIME_STEPS))
    plt.gcf().clear()
...
make_model(X, max_sentence_len, one_hot_subtask_a)
