...
def make_model(X, max_sentence_len, one_hot_subtask_b):
    ...
    #checkpoint
    filepath="weights-improvement-b-biLSTM-fasttextpretrained-{epoch:02d}-{val_acc:.2f}.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
    callbacks_list = [checkpoint]

    #fit the model
    model_Bi_LSTM.fit(x=X, y=one_hot_subtask_b, validation_split=0.33,	batch_size=64, epochs=30, callbacks=callbacks_list, verbose=0)
    ...
    plt.ylabel("acc")
    plt.legend()
    plt.savefig('Bi_LSTM_b | fasttext_trained')
