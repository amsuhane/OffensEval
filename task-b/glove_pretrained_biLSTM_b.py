...
def load_data():
    ...
    glove_vectors = np.load('../embeddings/glove_6B_300d.npy').item()
    ...
    return glove_vectors, data_training, corpus, sentences, one_hot_subtask_a, one_hot_subtask_b, one_hot_subtask_c
...
def sentence_word_to_vector():
    ...
    for i in sentences:
        for j in i:
            if j not in glove_vectors.keys():
    ...
    X_glove = [[glove_vectors[j] for j in i] for i in corpus_no_unknown]

    #Pad sentences
    max_sentence_len = np.max([len(i) for i in X_glove])
    padded = pad_sequences(X_glove, padding='post', dtype='object')
    X_word_embeddings_padded = padded

    return X_word_embeddings_padded, max_sentence_len

def make_model(X, max_sentence_len, one_hot_subtask_b):
    ...
    #fit the model
    model_Bi_LSTM.fit(x=X, y=one_hot_subtask_b, validation_split=0.33,	batch_size=64, epochs=50, callbacks=callbacks_list, verbose=0)
    ...
    plt.plot(model_Bi_LSTM.history.history['val_loss'], label = 'val_loss')
    plt.plot(model_Bi_LSTM.history.history['loss'], label = 'loss')
    plt.title("BiLSTM (subtask_b) model (loss='categorical_crossentropy', optimizer='Adam', metrics=['accuracy']) \n GloVe pre-trained-vectors")
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.legend()
    plt.subplot(1, 2, 2)
...
glove_vectors, data_training, corpus, sentences, one_hot_subtask_a, one_hot_subtask_b, one_hot_subtask_c = load_data()
X, max_sentence_len = sentence_word_to_vector()
make_model(X, max_sentence_len, one_hot_subtask_b)
