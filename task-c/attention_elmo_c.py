...
def load_data():
    ...
    not_off = []
    for i in range(len(data_training['subtask_c'])):
        if data_training['subtask_c'][i]==0:
            not_off.append(i)
...
def make_model(X, one_hot_subtask):
    ...
    output = Dense(3, activation="sigmoid")(attention_mul)
    ...
    #checkpoint
    filepath="weights-improvement-attention-elmo-c-{epoch:02d}-{val_acc:.2f}.hdf5"
    ...
    #fit the model
    model.fit(x=X, y=one_hot_subtask, validation_split=0.33,  batch_size=64, epochs=50, callbacks=callbacks_list, verbose=0)
    ...
    plt.title("elmo - Attention (subtask_c) model (loss='categorical_crossentropy', optimizer='Adam', metrics=['accuracy']) \n fasttext pre-trained-vectors")
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.legend()
    plt.subplot(1, 2, 2)
    ...
    plt.ylabel("acc")
    plt.legend()
    plt.savefig('elmo attention  c | fasttext')
...
print(4)
make_model(X, one_hot_subtask_c)
