...
def make_model(X, one_hot_subtask):
	...
	model = Sequential()
	model.add(Dense(256, input_dim=1024, kernel_regularizer=regularizers.l2(0.01), activity_regularizer=regularizers.l1(0.01)))
	model.add(Dense(4, activation='sigmoid'))
	model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
	model.fit(x=X, y=one_hot_subtask, validation_split=0.33,  batch_size=64, epochs=100, verbose=1)
	plt.subplot(1, 2, 1)
